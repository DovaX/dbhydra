...
class db(AbstractDB):
    def connect_remotely(self):
        ...
        self.connection = pyodbc.connect(
                            ...
                            r'UID=' + self.DB_USERNAME + ';'r'PWD=' + self.DB_PASSWORD + '',
            timeout=1
        )
    ...
    def insert(self, rows, batch=1, replace_apostrophes=True, try_mode=False, debug_mode=False):
        ...
        for k in range(len(rows)):
            ...
            for j in range(len(rows[k])):
                if rows[k][j] == "NULL" or rows[k][j] == "'NULL'" or rows[k][j] is None or \
                        rows[k][j] == "None":  # NaN hodnoty
                    query += "NULL,"
...
class MysqlTable(MysqlSelectable, AbstractTable):
    ...
    def insert(self, rows, batch=1, replace_apostrophes=True, try_mode=False, debug_mode=False):
        ...
        for k in range(len(rows)):
            ...
            query += "("
            for j in range(len(rows[k])):
                if rows[k][j] == "NULL" or rows[k][j] == "'NULL'" or rows[k][j] is None \
                        or rows[k][j] == "None":  # NaN hodnoty
